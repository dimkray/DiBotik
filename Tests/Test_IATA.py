...
with Profiler() as p:
    ...
    Test.Add(service+'.GetAirport','normal code',
             test, {'code': 'ACM', 'name': 'Арика'})

    test = IATA.GetAirport(code='AR')
    Test.Add(service+'.GetAirport','unreal code',
             test, {})

    test = IATA.Airport(code='ACM')
    etalon = [('ACM', 'Арика', 'Арика', 5.0, 'Колумбия', -2.133, -71.783, '', '', '', '', '')]
    Test.Add(service+'.Airport','normal code db', test, etalon)

    test = IATA.Airport(code='ACM', name='Шереметьево')
    Test.Add(service+'.Airport','normal code-name db', test, etalon)
    ...
    Test.Add(service+'.Airport','normal name db', test, etalon)
    ...
    Test.Add(service+'.Airport','unreal code db', test, [])

    test = IATA.Airport(name='Перьмь')
    Test.Add(service+'.Airport','unreal name db', test, [])

    test = IATA.Airport()
    Test.Add(service+'.Airport','crash db', test, [])

    # City
    test = IATA.GetCity(code='PEE')
    Test.Add(service+'.GetCity','normal code',
             test, {'code': 'PEE', 'country_code': 'RU', 'name': 'Пермь'})

    test = IATA.GetCity(code='EEE')
    Test.Add(service+'.GetCity','unreal code', test, {})
    ...
    Test.Add(service+'.City','normal code db', test, etalon)

    test = IATA.City(code='PEE', name='Москва')
    Test.Add(service+'.City','normal code-name db', test, etalon)
    ...
    Test.Add(service+'.City','normal name db', test, etalon)

    test = IATA.City(name='Нью-Йорк') # 11 аэропортов
    Test.Add(service+'.City','normal name big db', len(test), 11)

    test = IATA.City(code='EEE')
    Test.Add(service+'.City','unreal code db', test, [])

    test = IATA.City(name='Перьмь')
    Test.Add(service+'.City','unreal name db', test, [])

    test = IATA.City()
    Test.Add(service+'.City','crash db', test, [])

    # Country
    test = IATA.GetCountry(code='RU')
    Test.Add(service+'.GetCountry','normal code',
             test, {'code': 'RU', 'code3': 'RUS', 'iso_numeric': 643, 'name': 'Россия', 'languages': []})

    test = IATA.GetCountry(code='RUU')
    Test.Add(service+'.GetCountry','unreal code', test, {})

    test = IATA.Country(code='RU')
    etalon = [('RU', 'RUS', '643', 'Россия')]
    Test.Add(service+'.Country','normal code db', test, etalon)

    test = IATA.Country(name='росс')
    Test.Add(service+'.Country','normal name db', test, etalon)

    test = IATA.Country(code='EST', name='Россия')
    Test.Add(service+'.Country','normal code-name db', test, [('EE', 'EST', '233', 'Эстония')])

    test = IATA.Country(code='WW')
    etalon = [('RU', 'RUS', '643', 'Россия')]
    Test.Add(service+'.Country','unreal code db', test, [])

    test = IATA.Country(name='еее')
    Test.Add(service+'.Country','unreal name db', test, [])

    test = IATA.Country()
    Test.Add(service+'.Country','crush db', test, [])
