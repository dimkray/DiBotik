    ...
    for item in mtest:
        if i < len(metalon):
            ...
            for ietalon in metalon:
                if item == ietalon: rez += fl * abs(et-i)/len(metalon)
...
class Comp:
    ...
    def isType(testvalue, etalonvalue):
        if type(testvalue) == type(etalonvalue): return 1
        else: return 0
    
    # Сравнение с эталонным
    def Equal(test, etalon):
        if test == etalon: return 1
        else: return 0

    # Сравнение с эталонным (float)
    def fEqual(ftest, fetalon):
        try:
            if ftest == fetalon: return 1
            else:
                if abs(ftest - fetalon) / fetalon > 0.01: return 0
                else: return 1 - abs(ftest - fetalon) / (fetalon * 0.01)
        except Exception as e:
...
class Test:
    ...
    def Add(service, name, testvalue, etalonvalue, time=0, critery=1):
        ...
        iTest.append(name)
        iTest.append(str(testvalue))
        iTest.append(str(etalonvalue))
        ftest = 0; comm = ''
        ...
        iTest.append(ftest)
        if ftest >= critery: iTest.append(True)
...
class Report:
    ...
    def WriteFails(items=0):
        mreport = []
        for i in range(0,len(Tests)):
            if Tests[i][5] == False: mreport.append(Report.Write(i))
        if items == 0: items = len(mreport)
        return Fixer.strformat(mreport, items)
